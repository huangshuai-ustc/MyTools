            ...
            HStack(spacing: 10) {
                ...
                if presentation.hasBasePriceChart {
                    ForEach(selections) { selection in
                        Text(StockChartPresentation.plainPriceText(
                            NSDecimalNumber(decimal: selection.averagePrice).doubleValue
                        ))
                        .fontWeight(.semibold)
                ...
                if displayModes.contains(.volume) {
                    Text(point.volume.map(StockChartPresentation.volumeText) ?? "--")
                        .fontWeight(.medium)
                }
