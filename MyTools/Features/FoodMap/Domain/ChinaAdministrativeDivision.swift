...
struct ChinaAdministrativeDivision: Identifiable, Equatable, Sendable {
    let name: String
    let cities: [String]
...
enum ChinaAdministrativeDivisions {
    static let provinces: [ChinaAdministrativeDivision] = [
        division("北京市", ["北京市"]),
        division("天津市", ["天津市"]),
        division("河北省", [
        ...
        division("新疆维吾尔自治区", [
            ...
            "石河子市", "阿拉尔市", "图木舒克市", "五家渠市", "北屯市", "铁门关市",
            "双河市", "可克达拉市", "昆玉市", "胡杨河市", "新星市", "白杨市"
        ]),
        division("台湾省", [
            "台北市", "新北市", "桃园市", "台中市", "台南市", "高雄市", "基隆市", "新竹市",
            "嘉义市", "新竹县", "苗栗县", "彰化县", "南投县", "云林县", "嘉义县", "屏东县",
            "宜兰县", "花莲县", "台东县", "澎湖县", "金门县", "连江县"
        ]),
        division("香港特别行政区", ["香港特别行政区"]),
        division("澳门特别行政区", ["澳门特别行政区"])
    ]

    static func cities(in province: String) -> [String] {
    ...
    static func infer(from text: String) -> ChinaAdministrativeLocation? {
        ...
        for province in provinces {
            if let city = province.cities
                .sorted(by: { $0.count > $1.count })
                .first(where: text.contains) {
                return ChinaAdministrativeLocation(province: province.name, city: city)
            }
        }

        for province in provinces where text.contains(province.name) {
            if let city = province.cities.first(where: { text.contains(cityAlias(for: $0)) }) {
                return ChinaAdministrativeLocation(province: province.name, city: city)
            }
            if province.cities.count == 1, let city = province.cities.first {
                return ChinaAdministrativeLocation(province: province.name, city: city)
            }
        }

        if let province = provinces.first(where: { provinceAlias(for: $0.name) == text }) {
            guard province.cities.count == 1, let city = province.cities.first else { return nil }
            return ChinaAdministrativeLocation(province: province.name, city: city)
        }

        let candidates = Set(provinces.flatMap { province in
            province.cities.compactMap { city -> ChinaAdministrativeLocation? in
                let alias = cityAlias(for: city)
                guard alias.count >= 2, text.contains(alias) else { return nil }
                return ChinaAdministrativeLocation(province: province.name, city: city)
            }
        })
    ...
    private static func cityAlias(for city: String) -> String {
        let suffixes = ["特别行政区", "自治州", "地区", "林区", "市", "县", "盟"]
        guard let suffix = suffixes.first(where: city.hasSuffix) else { return city }
        return String(city.dropLast(suffix.count))
    }

    private static func provinceAlias(for province: String) -> String {
        let aliases = [
            "内蒙古自治区": "内蒙古",
            "广西壮族自治区": "广西",
            "西藏自治区": "西藏",
        ...
        if let alias = aliases[province] { return alias }
        if province.hasSuffix("省") || province.hasSuffix("市") {
            return String(province.dropLast())
        }
        return province
    }
    ...
    private static func division(
        _ name: String,
        _ cities: [String]
    ) -> ChinaAdministrativeDivision {
        ChinaAdministrativeDivision(name: name, cities: cities)
    }
}
