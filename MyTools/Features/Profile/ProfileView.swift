...
private struct StorageDataView: View {
        ...
        List {
            if let scanResult {
                Section {
                    ...
                    Text("仅统计我的工具箱在本机保存的应用数据，不包括 iOS 系统缓存和临时空间。")
                }
    ...
    private var referencedStoredFileNames: Set<String> {
        let medicalAttachments = store.medicalRecords.flatMap(\.attachments)
        let cardAttachments = store.cards.flatMap { card in
            card.statements.compactMap(\.attachment)
        }
        let secretAttachments = store.secretItems.flatMap(\.attachments)
