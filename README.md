...
“方寸”（英文名 `Fangcun`）是一个使用 SwiftUI 编写的本地优先多平台应用，界面语言为中文。应用把金融账户、股票、换汇、健康资料和保密资料集中保存在本机，适合个人长期记录、查询和备份。
...
- `AttachmentStore` 负责单个附件的文件生命周期；备份处理器负责健康、金融和保密资料的附件映射，三个模块共用同一套文件安全属性和预览包装器。
...
- 普通本地档案和保密资料目前没有应用层加密；项目包含独立 `MyToolsTests` Target，行情、备份、医疗规则和模块 Store 已有自动化回归测试，跨平台 UI 仍需设备验证。
